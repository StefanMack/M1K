...
#
# Version 1.0, first release version with new name "smuc" (source measurement
# unit control)
'''
Bugs:
- derzeit keine :-))
...
# *****************************************************************************
# smuc Version 1.0, S. Mack, 22.1.2021
# *****************************************************************************
...
from smucIcons import hipulse, lowpulse, oszi # Bilddateien der Icons
# Thinter UI Menüs
from smucMenus import MakeSettingsMenu, CreateToolTip, MakeAWGMenu
# Samplingfunktionen des M1K
import smucM1kSamp as m1k
# Oszillsokopfunktionen
from smucOsciFunc import (stop_samp, set_hscale, set_hpos,
...
cf.root.title("Dev-Version smuc v1.0 (22.1.2021)")
...
def onCanvasClickLeft(event):
    ...
    if cf.RUNstatus.get() == 0 and cf.ShowCur.get() > 0:
        ...
        if Tpoint >= 1000:
            ...
            TString = ' {0:.3f} '.format(axis_value) + " s, "
        ...
        if cf.MarkerNum > 1: # ab dem zweiten Marker
            ...
            if Tpoint >= 1000:
                ...
                DeltaT = ' {0:.3f} '.format(axis_value) + " s, "
            if Tpoint < 1000 and Tpoint >= 1:
                axis_value = DT
...
def ConSingDev():
    ...
    if cf.DevID == "No Device":
        ...
        if FWRevOne < 2.17:
            tkm.showwarning("WARNING","This smuc version Requires Firmware version > 2.16")
...
# Function to close and exit smuc
def Bcloseexit():
...
math_tip = CreateToolTip(mathmenu, 'Mathematikfunktionen auf Signalverläufe, Vertikalskala dazu ist die des ersten Operanden (z.B. Bezug CB-V/CA-V auf Skala von CB-V)')
...
# Tooltips
CreateToolTip(curvelab, 'Auswahl der dargestellten Signalverläufe. Für 200 kS/s maximal 2 Kanäle auswählen. (Kombi CA-V/CB-I und CB-V/CA-I nur max 100 kS/s.)')
...
# Tooltips
SampRate_tip = CreateToolTip(cf.SampRatesb, 'Abtastrate für ADC (Oszi) und DAC-Abtastrate (AWG). 200 kS/s nur bei ADC falls 2 oder weniger Kanäle aktiv. (Kombi CA-V/CB-I und CB-V/CA-I nur max 100 kS/s.)')
...
gain_hint = ttk.Label(frame2r, text="press <Return> to confirm" )
gain_hint.pack(side=tk.TOP, pady = (2,0)) 
# Tooltips
probe_tip = CreateToolTip(prlab, 'Korrektur von Gain- und Offsetfehler der vier Messkanäle zum Kalibrieren bzw. CA-V-Teilerverhältnis und -Offset Steckadapter')
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# AWG Menü
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
