...
class Scope(object):
    ...
    def __init__(self, ax, session, n_s_pts=400, red_rat=1, mode_a='Hi_Z',mode_b='Hi_Z'): 
        self.ax = ax
    ...
    def show_samples(self,vals):
        vals_a = vals[0][:: self.red_rat] # reduce effective sampling rate
        vals_b = vals[1][:: self.red_rat] # reduce effective sampling rate
...
try:
    ...
    if session.devices:
        ...
        fig, ax = plt.subplots()
        my_scope = Scope(ax,session,n_s_pts=N_S_PTS, red_rat=RED_RAT, mode_a='Hi_Z', mode_b='Hi_Z')
