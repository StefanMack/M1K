    ...
    for freq in range(START_FREQ,STOP_FREQ,FREQ_STEP):
        if(keep_going):
            ...
            if first_step:
                ...
                time.sleep(10)
                print("Frequenz (HZ) Spannung (V) Strom (mA) Widerstand (Ohm)")
                print("------------------------------------------------------")
                first_step = False
            ...
            print("t={:4}: f={:4} DC: U={:6.3f} V, I={:7.4f} mA, R={:7.3f} Ohm   AC: U={:8.6f} V, I={:7.4f} mA, R={:8.4f} Ohm, C={:4.1f} mAh"\
            .format(meas_time, freq, u_ave, i_ave*1000, r_ave, u_std, i_std*1000, r_std, charge_val))
