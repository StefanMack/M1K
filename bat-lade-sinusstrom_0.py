...
über einen Vorwiderstand von ca. 0,5 Ohm aufgeladen wird.

Ausgegeben wird jeweils der Mittelwert und die Standardabweichung der Spannung
von Kanal A (Akkuspannung) und des Stroms von Kanal B (Ladestrom).
...
meas_file.write('t [s]; U_DC [v]; I_DC [mA]; R_DC [Ohm]; U_AC [v]; I_AC [mA]; R_Diff [Ohm] \n\n')
...
if session.devices:
    ...
    while keep_going:
        ...
        i_ave = chb_i_vals.mean() - 0.000 # Offset correction
        r_ave = u_ave / i_ave
        ...
        print("t={:4}: DC: U={:6.3f} V, I={:7.4f} mA, R={:7.3f} Ohm   AC: U={:8.6f} V, I={:7.4f} mA, R={:8.4f} Ohm".format(meas_time, u_ave, i_ave*1000, r_ave, u_std, i_std*1000, r_std))
        meas_file.write("{};{:.3f};{:.4f};{:.3f};{:.6f};{:.4f};{:.4f}\n".format(meas_time, u_ave, i_ave*1000, r_ave,u_std, i_std*1000, r_std))
        time.sleep(2)
